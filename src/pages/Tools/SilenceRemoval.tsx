...
import { removeSilence } from '../../utils/audioAdvanced'
...
const SilenceRemovalPage: React.FC = () => {
  ...
  const convertOne = async (item: FileItem) => {
    try {
      ...
      const blob = await removeSilence(item.file, item.file.type || 'audio/mpeg')
      ...
      setFiles((prev) =>
        prev.map((f) =>
          f.id === item.id
            ? { ...f, status: 'error', errorMessage: e?.message || 'Silence removal failed' }
            : f
        )
      )
  ...
  const downloadOne = (item: FileItem) => {
    ...
    a.href = item.url
    a.download = item.name.replace(/\.[^.]+$/, '') + `_cleaned.mp3`
    a.click()
  }
  ...
  return (
    <>
      <SEO
        title="Online Silence Remover - Clean Audio"
        description="Remove silence and light background noise from your audio files directly in your browser."
        path="/tools/silence-removal"
      />
      <div className="page-container">
        <h1 className="page-title">Silence Remover</h1>
        <p className="page-subtitle">Detect leading silence and apply light noise reduction, ideal for podcasts and recordings.</p>

        <section style={{ marginTop: 24 }}>
          <Card>
            <div className="upload-area">
              <p className="upload-hint">Click or drag audio files here (voice recordings, podcasts, meeting audio, etc.)</p>
              <input type="file" accept="audio/*" multiple onChange={handleInputChange} />
            </div>

            <p className="field-help">
              This tool attempts to: 1) reduce constant background noise; 2) remove short leading silence. Processing is done locally in your browser.
            </p>
...
export default SilenceRemovalPage
